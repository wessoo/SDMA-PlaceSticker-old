...
package id.element
{
	...
	public class FlickrParser extends EventDispatcher
	{
		...
		{
			type=settings.GlobalSettings.type;
			if (type=="group")
			{
			...
			vars.api_key=settings.GlobalSettings.apiKey;
			request.data=vars;
		...
		private static function addSizes(e:Event):void
		{
			...
			object.imgUrl=sizeXml.sizes.size[sizeXml.sizes.size.length()-1].@source;
			object.videoSource=null;

			var label:Object=sizeXml.sizes.size[sizeXml.sizes.size.length()-1].@label;
			if (label=="Mobile MP4")
			{
